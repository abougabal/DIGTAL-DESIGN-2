...
void leffile::set_start(string x) {
	...
	if (openfile.is_open())
	{
		...
		while (temp2 != "END LIBRARY") {
			//the header
			while (temp != "SITE" && temp != "LAYER" && temp != "MACRO" && temp2 != "END LIBRARY"&&flag) {
			...
			flag = false;//to prevent re entering the code if we have other parts in the lef file other than what we have or mentioned
